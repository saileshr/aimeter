#!/usr/bin/env python3
"""AgentMeter Model Comparison Demo
...
from agentmeter import MemoryExporter, configure, record_outcome
from agentmeter.report import print_report
...
def main() -> None:
    # Set up AgentMeter with memory exporter
    mem = MemoryExporter()
    configure(project="model-comparison", exporters=[mem])

    print()
    print("=" * 60)
    print("  AgentMeter Model Comparison Demo")
    ...
    print_report(
        llm_events,
        title="AgentMeter — Model Cost Comparison",
    )
...
def _print_outcome_summary(mem: MemoryExporter) -> None:
    ...
    print(f"{DIM}  Powered by AgentMeter — agentmeter.ai{RESET}")
    print()
