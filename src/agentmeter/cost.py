...
class CostRegistry:
    ...
    def calculate(self, provider: str, model: str, tokens: TokenUsage) -> CostBreakdown:
        ...
        if pricing is None:
            logger.debug(
                "agentmeter: unknown model '%s/%s' — cost will be $0.00. "
    ...
    def update_from_url(self, url: str, timeout: float = 10.0) -> int:
        ...
        The JSON must be in the agentmeter dict format:
        ...
        """
        req = urllib.request.Request(url, headers={"User-Agent": "agentmeter-sdk"})
    ...
    def _import_litellm_format(self, raw: dict[str, Any]) -> int:
        ...
        logger.info("agentmeter: loaded %d models from litellm registry", count)
        return count
